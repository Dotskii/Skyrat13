	...
	if (suspension_field)
		...
		if(field_type == "carbon")
			for(var/mob/living/carbon/M in T)
				var/K = M.AmountKnockdown()
				M.SetKnockdown(max(K, 3))
				var/S = M.AmountStun()
				M.SetStun(max(S, 3))
				cell.charge -= power_use
				if(prob(5))
...
/obj/machinery/suspension_gen/proc/activate()
	...
	switch(field_type)
		if("carbon")
			...
			for(var/mob/living/carbon/C in T)
				C.AdjustKnockdown(5)
				C.visible_message("<span class='notice'>[icon2html(C, viewers(7, get_turf(C)), C.icon_state)] [C] begins to float in the air!</span>","You feel tingly and light, but it is difficult to move.")
		if("nitrogen")
			success = 1
			//
		if("mercury")
		...
		if("iron")
			...
			for(var/mob/living/silicon/R in T)
				R.AdjustKnockdown(5)
				R.visible_message("<span class='notice'>[icon2html(R, viewers(7, get_turf(R)), R.icon_state)] [R] begins to float in the air!</span>","You feel tingly and light, but it is difficult to move.")
			//
	...
	src.visible_message("<span class='notice'>[icon2html(src, viewers(7, get_turf(src)), src.icon_state)] \the [src] activates with a low hum.</span>")
	...
	if(collected)
		suspension_field.icon_state = "energynet"
		suspension_field.overlays += image(icon = suspension_field.icon, icon_state = "shield2")
		src.visible_message("<span class='notice'>[icon2html(suspension_field, viewers(7, get_turf(suspension_field)), suspension_field.icon_state)] [suspension_field] gently absconds [collected > 1 ? "something" : "several things"].</span>")
	else
		if(istype(T,/turf/closed/mineral) || istype(T,/turf/closed/wall))
...
/obj/machinery/suspension_gen/proc/deactivate()
	...
	src.visible_message("<span class='notice'>[icon2html(src, viewers(7, get_turf(src)), src.icon_state)] \the [src] deactivates with a gentle shudder.</span>")

...
/obj/item/device/core_sampler
	...
	icon = 'modular_skyrat/icons/obj/xenoarch/device.dmi'
	icon_state = "sampler0"
	item_state = "screwdriver_brown"
	w_class = WEIGHT_CLASS_TINY
	slot_flags = SLOT_BELT
	var/num_stored_bags = MAX_STORED_BAGS
	var/obj/item/storage/evidencebag/sample/filled_bag
...
/obj/item/device/core_sampler/attackby(obj/item/W, mob/user)
	if(istype(W,/obj/item/storage/evidencebag/sample))
		insert_bag_maybe(W, user)
		return TRUE
	else
		return ..()

/obj/item/device/core_sampler/proc/insert_bag_maybe(obj/item/storage/evidencebag/sample/W, mob/user)
...
/obj/item/device/core_sampler/proc/sample_item(var/item_to_sample, var/mob/user)
	...
	if(geo_data)
		if(filled_bag)
			...
			filled_bag = new /obj/item/storage/evidencebag/sample(src)
...
/obj/item/storage/evidencebag/sample/attackby(obj/item/W, mob/user)

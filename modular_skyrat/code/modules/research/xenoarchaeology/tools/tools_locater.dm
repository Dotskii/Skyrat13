	...
	if(target_radio)
		...
		if(scan_ticks)
			...
			if(prob(scan_ticks * 10))
				spawn(0)
					//set background = 1
					if(SSprocessing.processing.Find(src))
						...
						if(target_radio)
							T.visible_message("[icon2html(src, usr, icon_state)] [src] [pick("chirps","chirrups","cheeps")] happily.")
						else
							T.visible_message("[icon2html(src, usr, icon_state)] [src] [pick("chirps","chirrups","cheeps")] sadly.")
		else
			icon_state = "pinoff"

...
/obj/item/device/depth_scanner
	...
	slot_flags = SLOT_BELT | SLOT_L_STORE | SLOT_R_STORE | SLOT_S_STORE
...
/obj/item/device/depth_scanner/proc/scan_atom(var/mob/user, var/atom/A)
	...
	if(istype(A,/turf/closed/mineral))
		...
		if(M.finds.len || M.artifact_find)
			...
			D.coords = "[M.x[M.z]].[rand(0,9)]:[M.y[M.z]].[rand(0,9)]:[10 * M.z].[rand(0,9)]"
			D.time = worldtime2text()
			D.record_index = positive_locations.len + 1
			D.material = M.mineralType ? M.mineralType.name : "Rock"
			...
			for(var/mob/L in range(src, 1))
				to_chat(L, "<span class='notice'>[icon2html(src, L, icon_state)] [src] pings.</span>")
		...
		if(B.artifact_find)
			//create a new scanlog entry
			var/datum/depth_scan/D = new()
			D.coords = "[10 * (B.x[B.z])].[rand(0,9)]:[10 * (B.y[B.z])].[rand(0,9)]:[10 * B.z].[rand(0,9)]"
			D.time = worldtime2text()
			D.record_index = positive_locations.len + 1
			...
			for(var/mob/L in range(src, 1))
				to_chat(L, "<span class='notice'>[icon2html(src, L, icon_state)] [src] pings [pick("madly","wildly","excitedly","crazily")]!</span>")

...
/obj/machinery/bunsen_burner
	...
	var/list/possible_fuels = list(
		PLASMA = list(
				"max_temperature" = PLASMA_UPPER_TEMPERATURE,
...
/obj/machinery/bunsen_burner/Initialize()
	. = ..()
...
/obj/machinery/bunsen_burner/attackby(obj/item/W, mob/user)
	if(istype(W, /obj/item/reagent_containers))
		...
		if(heating == BUNSEN_OPEN && R.is_open_container())
			...
			if(!held_container && user.transferItemToLoc(W, src))
	...
	if(W.tool_behaviour == TOOL_WRENCH)
		...
		if(do_after(user, src, 5 SECONDS))
			...
			var/randumb = rand(3,4)
			for(var/i = 0, i >= randumb, i++)
...
/obj/machinery/bunsen_burner/proc/load_item(obj/item/W)
	held_container = W
	var/image/I = image("icon"=W, "layer"=FLOAT_LAYER, "pixel_x" = 2, "pixel_y" = 22 - empty_Y_space(new /icon(W.icon, W.icon_state)))
